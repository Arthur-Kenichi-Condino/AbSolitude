...
namespace AKCondinoO.Voxels.Terrain.MarchingCubes{
    ...
    internal class MarchingCubesTerrain{
     ...
     internal static ConcurrentQueue<List<Vector2>>vertexUVListPool=new ConcurrentQueue<List<Vector2>>();
        ...
        internal static void DoPredictionMarchingCubes(
         Voxel[]polygonCell,
          Vector3Int vCoord1,
           Vector3[]vertices,
            //Vector3[][][]verticesCache,
        ...
        internal static bool TryGetSlopeHitNormalAtPoint(
         Vector3 worldPoint,
         List<Vertex>TempVer,
         List<UInt32>TempTri,
         Vector2Int cnkRgn,
         ...
         for(int i=0;i<TempTri.Count;i+=3){
          uint idx_a=TempTri[i  ];
          uint idx_b=TempTri[i+1];
          uint idx_c=TempTri[i+2];
          Vector3 a=TempVer[(int)idx_a].pos;
          Vector3 b=TempVer[(int)idx_b].pos;
          Vector3 c=TempVer[(int)idx_c].pos;
          //  primeiro: projeção 2D XZ
          if(!PointInsideTriangleXZ(localPoint,a,b,c))continue;
          //  segundo: calcula a interseção vertical com o plano ABC
          if(!TryGetVerticalIntersection(localPoint,a,b,c,out float yHit))continue;
         ...
         static bool PointInsideTriangleXZ(Vector3 p,Vector3 a,Vector3 b,Vector3 c){
          ...
          if(Mathf.Abs(denom)<1e-8f)return false;//  tri muito pequeno ou degenerado
          float invDenom=1.0f/denom;
          float u=(dot11*dot02-dot01*dot12)*invDenom;
          float v=(dot00*dot12-dot01*dot02)*invDenom;
          return(u>=0)&&(v>=0)&&(u+v<=1);
         }
         static bool TryGetVerticalIntersection(
          Vector3 p,
          Vector3 a,
          Vector3 b,
          Vector3 c,
          ...
          if(n.magnitude<1e-8f)return false;//  tri degenerado
          //  queremos resolver só Y:
          if(Mathf.Abs(n.y)<1e-8f)return false;//  plano quase vertical: impossível para ray vertical
          float t=(-n.x*(p.x-a.x)-n.z*(p.z-a.z))/n.y;
          yHit=a.y+t;
          return true;
         }
