     ...
     private static Snapshot snapshot;
        ...
        internal static void SetTerrainvxl(ref Voxel vxl,Vector3Int vCoord,Vector2Int cCoord,BiomesConfigurationContext context){
         ...
         if(!context.TryUseTerrainHeightNoiseCache(out heightValue)){
          heightValue=snapshot.graph.GetValue(NoiseChannel.TerrainHeight,new(noiseInput.z,noiseInput.x,0));
          context.UpdateTerrainHeightNoiseCache(heightValue);
         }
         if(heightValue>=0d){
          var materialTable=snapshot.graph.GetMaterialTable(NoiseChannel.TerrainSurfaceMaterial,new(noiseInput.z,noiseInput.x,0));
